...
def make_progress_bar(pct, width=20):
    """Render a simple text progress bar for a 0-100 percentage value."""
    try:
        pct_val = float(pct)
    except (TypeError, ValueError):
        pct_val = 0.0
    clamped = max(0.0, min(100.0, pct_val))
...
def print_dashboard(timestamp, labour, equipment, material):
    ...
    print(f"  Availability: {make_progress_bar(labour['availability_pct'])}   Shortage: {labour['shortage']}")
    print("  Zone-wise:")
    for z in labour["zone_availability"]:
        print(
            f"    {z['Zone']:<8} Available={z['Available_Labour']:<5} "
            f"Required={z['Required_Labour']:<5} "
            f"{make_progress_bar(z['Availability_%'])} Shortage={z['Shortage']:<5} Status={z['Zone_Status']}"
        )

    print("\n[EQUIPMENT]")
    print(f"  Available: {equipment['total_available']} / Required: {equipment['total_required']}")
    print(f"  Availability: {make_progress_bar(equipment['availability_pct'])}   Shortage: {equipment['shortage']}")
    ...
    for z in equipment["zone_availability"]:
        print(
            ...
            f"{make_progress_bar(z['Availability_%'])} Shortage={z['Shortage']:<5} "
    ...
    print(f"  Availability: {make_progress_bar(material['availability_pct'])}   Shortage: {material['shortage']}")
    print("  Zone-wise:")
